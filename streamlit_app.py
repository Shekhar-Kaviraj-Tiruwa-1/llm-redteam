"""
LLM Safety Evaluator Comparison Dashboard
==========================================

Entry point for Streamlit Community Cloud deployment.

Reads from data/results_method3.db (committed to repo).
No API keys or external services needed — read-only dashboard.
"""
...
# ── DB path — relative to this file so it works both locally and on Streamlit Cloud ──
_ROOT   = os.path.dirname(os.path.abspath(__file__))
DB_PATH = os.path.join(_ROOT, "data", "results_method3.db")

# ── Page config ────────────────────────────────────────────────────────────────

st.set_page_config(
    page_title="LLM Safety Evaluator Comparison",
    page_icon="🔬",
    layout="wide",
)
...
st.title("🔬 LLM Safety Evaluator Comparison")
st.markdown(
    """
    **Can automated tools reliably judge whether an LLM responded safely to an adversarial prompt?**

    This project ran **488 adversarial prompts** from three public datasets
    ([StrongREJECT](https://github.com/alexandrasouly/strongreject),
    [JailbreakBench](https://jailbreakbench.github.io/), and a custom set)
    through two target models — **GPT-4o-mini** and **Claude 3.5 Haiku** —
    and then scored every response with *four independent safety evaluators simultaneously*.
    The core question: do these tools agree?
    """
)

with st.expander("📖 The Four Evaluators", expanded=True):
    c1, c2, c3, c4 = st.columns(4)
    c1.markdown(
        "**Keyword Filter**\n\n"
        "Rule-based heuristic. Looks for explicit refusal phrases "
        "(\"I can't\", \"I won't\") and hedging language. Fast, interpretable, no API call."
    )
    c2.markdown(
        "**LLaMA Guard 3**\n\n"
        "Meta's open-source content-safety classifier (8B). "
        "Trained on the MLCommons taxonomy of 13 harm categories. "
        "Returns a binary safe/unsafe label plus a violation code."
    )
    c3.markdown(
        "**LLM Judge**\n\n"
        "GPT-4o-mini prompted with the StrongREJECT rubric. "
        "Assigns a 0–1 harmfulness score and a free-text rationale. "
        "Most context-sensitive of the four."
    )
    c4.markdown(
        "**OpenAI Moderation API**\n\n"
        "OpenAI's `omni-moderation-latest` endpoint. "
        "Scores across 11 harm categories; threshold ≥ 0.5 → unsafe. "
        "Catches explicit content, self-harm, violence, etc."
    )
...
h1.metric("LLaMA Guard 'unsafe' rate — GPT-4o-mini",      "97.1%",
          help="LLaMA Guard flagged 97.1% of all GPT-4o-mini responses as unsafe")
h2.metric("LLaMA Guard 'unsafe' rate — Claude 3.5 Haiku", "96.4%",
          help="Nearly identical rate on a completely different model")
h3.metric("LLM Judge 'unsafe' rate (same prompts)",        "~3%",
          help="The LLM Judge found only ~3% unsafe — a 30× gap vs LLaMA Guard")

st.info(
    "LLaMA Guard fires **~97% 'unsafe'** on both models, while the LLM Judge finds only **~3%**. "
    "The other two evaluators sit in between but disagree with each other too. "
    "This pattern holds across both target models, confirming it is a property of "
    "**LLaMA Guard's calibration** — not of the models' actual behaviour. "
    "All four evaluators agree on only **~47%** of prompts."
)

st.markdown(
    "_Use the sections below to explore the full dataset interactively. "
    "Section 0 compares both target models; Sections 1–4 let you drill into a single run._"
)
...
# ── Identify the two primary 488-prompt runs for Model Comparison ─────────────
...
st.title("0 · GPT-4o-mini vs Claude 3.5 Haiku — Same Prompts, Same Evaluators")
...
if GPT_RUN_ID and HAIKU_RUN_ID:
    ...
    st.markdown("---")
    st.markdown("#### LLaMA Guard outlier pattern is evaluator-structural")
    ...
    st.info("Model comparison requires both the GPT-4o-mini and Claude 3.5 Haiku full runs (488 prompts each).")
...
st.caption(
    ...
    "Source: [github.com/shekhartiruwa](https://github.com/shekhartiruwa)"
)
